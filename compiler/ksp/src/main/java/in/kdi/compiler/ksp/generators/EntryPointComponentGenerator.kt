...
import `in`.kdi.compiler.ksp.TypeCollector
import `in`.kdi.compiler.ksp.ext.getAllRetrievers
...
private fun KSClassDeclaration.extractProperties() = sequence {
	...
	for (property in getAllProperties()) {
		...
		if (!property.hasAnnotation(`in`.kdi.runtime.annotations.Inject::class)) {
			continue
		}
